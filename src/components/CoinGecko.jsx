...
const CoinGecko = () => {
  ...
  if (loading) return <div className="text-center py-8 text-gray-300">Loading Market Data...</div>;
  ...
  return (
    <div className="h-full flex flex-col overflow-y-auto hide-scrollbar">
      <div className="overflow-x-auto mb-6 flex-shrink-0">
        <table className="min-w-full text-sm">
          <thead className="text-xs text-gray-300 uppercase">
            <tr>
              <th className="px-4 py-2 text-left text-gray-300">Coin</th>
              <th className="px-4 py-2 text-left text-gray-300">Price (USD)</th>
              <th className="px-4 py-2 text-left text-gray-300">1h %</th>
              <th className="px-4 py-2 text-left text-gray-300">24h %</th>
              <th className="px-4 py-2 text-left text-gray-300">7d %</th>
            </tr>
          </thead>
                ...
                <td className="px-4 py-3 whitespace-nowrap">
                  <div className="flex items-center">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-gray-300 uppercase">{coin.symbol}</div>
                    </div>
                  </div>
                </td>
                <td className="px-4 py-3 whitespace-nowrap font-semibold text-gray-200">${coin.current_price?.toLocaleString()}</td>
      ...
      <div className="flex-1 ">
        ...
        <div className="p-4 bg-black rounded-lg border border-gray-700 space-y-3 text-gray-200">
          {summaryLoading && <p className="text-gray-300">Generating AI market insights...</p>}
          ...
          {!summaryLoading && !summaryError && summaries.length > 0 ? (
            ...
            !summaryLoading && <p className="text-gray-400">No summary available.</p>
          )}
        </div>
      </div>
    </div>
